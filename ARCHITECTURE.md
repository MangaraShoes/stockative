...
O plano Starter inclui um número fixo de posts/mês (~12, 3/semana) e um número bem menor de créditos de imagem de IA/mês (~5) — ver preço em [CLAUDE.md](CLAUDE.md). Isso cria uma pergunta que o Estágio 1 precisa responder em nível de semana: *dos posts planejados, qual merece gastar um crédito de imagem?*

Regra v0 (baseada em regra, não aprendida — mesma limitação de cold start da Camada 3): ranquear os posts da semana pela prioridade já calculada por Commerce Intelligence (estoque/vendas) + relevância de calendário + peso do objetivo comercial. O post de maior prioridade da semana recebe `uses_ai_image: true`; os demais usam a fotografia já existente da loja (`creative_assets.source = shopify_existing`). O merchant pode sobrepor essa escolha manualmente ou comprar crédito extra.
...
- **Quota de posts** (~12/mês no Starter): conta `content_items` com `status IN (scheduled, published)` dentro do ciclo de cobrança. Regenerar legenda/objetivo várias vezes num rascunho não consome quota — só conta quando o conteúdo entra de fato no calendário/publicação.
...
**Alocação de crédito por semana**: ver "Planejamento semanal e alocação de crédito de imagem" na seção do Decision Engine acima — nem todo post da semana ganha imagem de IA, só o de maior prioridade dentro do orçamento do plano.
