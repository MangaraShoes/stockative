...
- Escopos confirmados **direto no painel do app na Meta** (mais confiável que busca na web, que deu nomes diferentes antes disso): `pages_show_list`, `pages_read_engagement`, `business_management`, `instagram_basic`, `instagram_content_publishing`. **Nem `instagram_content_publish` nem `instagram_business_content_publish` são o nome certo** — os dois apareceram em buscas anteriores e nenhum bate com o que o painel realmente pede.
